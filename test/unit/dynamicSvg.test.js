...
!developmentChains.includes(network.name)?describe.skip:describe("Dynamic Svg Unit test", function(){
    let deployer, dynamicNft, mockV3Aggregator;
    ...
    describe("constructor", function() {
        it("initializes Dynamic NFT correctly", async function(){
    ...
    describe("mintNft", async function(){
        it("emits an event after NFT is minted and shifts the tokenURI to high value once it's surpassed", async function(){
            const value = await ethers.utils.parseEther("1");
            await expect(dynamicNft.mintNft(value)).to.emit(dynamicNft, "CreatedNft");
            const tokenCounter = await dynamicNft.getTokenCounter();
            assert.equal(tokenCounter, "1");
            const tokenUri = await dynamicNft.tokenURI(0);
            assert(tokenUri, highTokenUri);
        });

        it("Shifts the tokenURI to low value if not surpassed", async function(){
            const value = await ethers.utils.parseEther("1000000000");
            const tx = await dynamicNft.mintNft(value);
            await tx.wait(1);
            const tokenUri = await dynamicNft.tokenURI(0);
            assert(tokenUri, lowTokenUri);
        });
    });
});